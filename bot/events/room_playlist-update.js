...
const lastfm = require(process.cwd()+ '/bot/utilities/lastfm.js');
...
/**
 * Scrobble the previous song to Last.fm and update Now Playing for the new song.
 * Scrobbling uses the stored start time of the previous song as the timestamp.
 *
 * @param {Object} bot
 * @param {Object} previousSong  mediaStore current (about to become last)
 * @param {Object} newSongName   combined "Artist - Title" name string of new song
 */
function handleLastfm(bot, previousSong, newSongName) {
  // Scrobble the previous song if we have valid data for it
  if (previousSong && previousSong.name && previousSong.when) {
    var prev = lastfm.parseName(previousSong.name);
    lastfm.scrobble(prev.artist, prev.track, previousSong.when, function(err) {
      if (err) {
        bot.log('warn', 'BOT', '[Last.fm] scrobble failed: ' + err.message);
      } else {
        bot.log('info', 'BOT', '[Last.fm] scrobbled: ' + previousSong.name);
      }
    });
  }

  // Update Now Playing for the new song
  if (newSongName) {
    var next = lastfm.parseName(newSongName);
    lastfm.updateNowPlaying(next.artist, next.track, function(err) {
      if (err) {
        bot.log('warn', 'BOT', '[Last.fm] updateNowPlaying failed: ' + err.message);
      } else {
        bot.log('info', 'BOT', '[Last.fm] now playing: ' + newSongName);
      }
    });
  }
}
...
module.exports = function(bot, db) {
  bot.on(bot.events.roomPlaylistUpdate, function(data) {
    ...
    /************************************************************
     * Last.fm scrobble previous song and update now playing
     */
    handleLastfm(bot, currentSong, newSong.name);
	...
	// This is broken.  No time to fix it, so I'm disabling -funkyfresh
      //if (result.skippable) {
