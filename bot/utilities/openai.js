...
/*
  ...
  This module:
    ...
    4. When the user asks about logs, songs, or history, injects the last
       LOG_TAIL_LINES lines of the pm2 out log as context.
*/

var LOG_PATH = '/root/.pm2/logs/DerpyBot-out.log';
var LOG_TAIL_LINES = 100;

// Keywords that trigger log injection into the OpenAI context
var LOG_KEYWORDS = [
  'log', 'logs', 'song', 'songs', 'played', 'playing',
  'history', 'last', 'recent', 'track', 'tracks', 'artist'
];
...
/**
 * Check if a message contains any log-related keywords.
 * @param {string} message
 * @returns {boolean}
 */
function isLogQuery(message) {
  var lower = message.toLowerCase();
  return LOG_KEYWORDS.some(function(kw) { return lower.indexOf(kw) !== -1; });
}
...
module.exports = function(bot, db, data) {
  ...
  // Build messages array: system + optional log context + history + new user message
  var messages = [{ role: 'system', content: systemPrompt }];

  if (isLogQuery(userMessage)) {
    var logContent = tailFile(LOG_PATH, LOG_TAIL_LINES);
    if (logContent) {
      messages.push({
        role: 'system',
        content: 'Here are the last ' + LOG_TAIL_LINES + ' lines of the bot log for context. ' +
                 'Song plays appear as lines ending in "saved to playlist", formatted as ' +
                 '"Artist - Title saved to playlist". ' +
                 'Use this to answer questions about recently played songs or room activity:\n\n' + logContent
      });
      bot.log('info', 'BOT', '[OpenAI] injecting log context for log-related query');
    } else {
      bot.log('warn', 'BOT', '[OpenAI] log context requested but log file could not be read');
    }
  }

  messages = messages.concat(history).concat([{ role: 'user', content: userMessage }]);
